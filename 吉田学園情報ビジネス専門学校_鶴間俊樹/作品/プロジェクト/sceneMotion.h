//=============================================================================
//
// モーションオブジェクト処理 [sceneMotion.h]
// Author : 鶴間俊樹
//
//=============================================================================
#ifndef _SCENE_MOTION_H_
#define _SCENE_MOTION_H_
...
// モーションオブジェクトクラス
//*****************************************************************************
class CSceneMotion : public CScene
{
public:
	//パーツ情報の構造体
	typedef struct {
		CModel::MODELTYPE modelType;	//パーツのモデルのタイプ
		int nIdx;			//パーツのインデックス 必要ないかも
	...
	CSceneMotion();		//デフォルトコンストラクタ
	CSceneMotion(const PARTS_INFO* pPartsInfoArray, int nNumParts, const MOTION_INFO* pMotionInfoArray, int nNumTypeMotion, bool bOutline);		//オーバーロードされたコンストラクタ
	~CSceneMotion();	//デストラクタ
	static void Load(char* pLoadDirectory, PARTS_INFO** ppPartsInfoArray, int* pNumParts, MOTION_INFO* pMotionInfoArray, int nNumMotionType, int* pIdxWeapon);	//モーション情報をロード
	...
	void Attack(OBJ_TYPE objtype, D3DXVECTOR3 pos, float fRadius, int nDamage, CSceneMotion::DAMAGE_TYPE typeDamage, int* pNumKill);	//攻撃
	...
	int GetCurrentKey(void);		//現在のモーションのキーの取得
